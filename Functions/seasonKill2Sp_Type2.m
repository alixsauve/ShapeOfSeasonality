function xdot = seasonKill2Sp_Type2(tt, xt, scale)
	% seasonKill2Sp_Type2() is the system of differential equation for the endogeneous model: a generalised Lotka-Volterra model for 2 species (type II functional responses)
	...
	x1dot = xt(1) * (c * a * fAR * xt(2) / (1 + a * h * fAR * xt(2)) - d); % predator's growth rate
	x2dot = xt(2) * (r - r * xt(2) / K - a * fAR * xt(1) / (1 + a * h * fAR * xt(2))); %prey's growth rate
  	xdot = [x1dot; x2dot];
end
